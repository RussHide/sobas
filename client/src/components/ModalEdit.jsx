...
const ModalEdit = ({ fetchPedidos, openEdit, setModals }) => {
  const btnEdit = async () => {
    const validarCampos = Object.values(openEdit.pedido).every(
      value => value !== ''
    )
    if (!validarCampos) {
      ...
      try {
        await axios.put('http://localhost:3000/pedidos/' + openEdit.pedido.id, openEdit.pedido)
        fetchPedidos()
        ...
        <div className="flex  justify-around items-center  w-full ">
          <div className="grid  grid-cols-1 md:grid-cols-3 gap-6 w-full  p-2">
            <div className="w-full col-span-1">
              ...
              <Input placeholder="" value={openEdit.pedido.cliente} onChange={e => setModals(prev => ({ ...prev, edit: { ...prev.edit, pedido: { ...prev.edit.pedido, cliente: e.target.value } } }))} />
            </div>
            <div className="flex col-span-1">
              <DatePicker
                ...
                locale="es"
                onChange={(date) => setModals(prev => ({ ...prev, edit: { ...prev.edit, pedido: { ...prev.edit.pedido, fechaLlegada: date.toLocaleDateString('sv') } } }))}
                customInput={<CustomDatePicker placeholer='Seleccionar fecha' label="Fecha Llegada" />}
              />
            </div>
            <div className="flex col-span-1">
              <DatePicker
                ...
                dateFormat="dd-MM-yyyy"
                locale="es"
                onChange={(date) => setModals(prev => ({ ...prev, edit: { ...prev.edit, pedido: { ...prev.edit.pedido, fechaEntrega: date.toLocaleDateString('sv')} } }))}
                customInput={<CustomDatePicker placeholer='Seleccionar fecha' label="Fecha Entrega" />}
              />
            </div>
            <div className="w-full col-span-1">
            ...
            <div className="w-full col-span-1">
              ...
              <Input placeholder="" value={openEdit.pedido.reparacion} onChange={e => setModals(prev => ({ ...prev, edit: { ...prev.edit, pedido: { ...prev.edit.pedido, reparacion: e.target.value } } }))} />
            </div>
            <div className="w-full col-span-1">
              <label className="text-gray-800 font-semibold px-1">Modelo</label>
              <Input placeholder="" value={openEdit.pedido.modelo} onChange={e => setModals(prev => ({ ...prev, edit: { ...prev.edit, pedido: { ...prev.edit.pedido, modelo: e.target.value } } }))} />
            </div>
            <div className="w-full col-span-1">
              <label className="text-gray-800 font-semibold px-1">Falla</label>
              <Input placeholder="" value={openEdit.pedido.falla} onChange={e => setModals(prev => ({ ...prev, edit: { ...prev.edit, pedido: { ...prev.edit.pedido, falla: e.target.value } } }))} />
            </div>
          </div>
        </div>
      </div>
